(ns weighpkgmfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control assembly/calibration-line
  equipment directly... does NOT authorize or execute maintenance/
  equipment operations... does NOT self-issue an OIML R76/NIST
  Handbook 44 legal-for-trade weighing-accuracy certification or a
  CE/ANSI-PMMI machinery-safety conformity mark') implemented
  faithfully. The single invariant under test:

    WeighPkgAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the General-Purpose Machinery
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-type :platform-scale}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :platform-scale (:product-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest certification-authority-is-held-and-permanently-blocked
  (testing "a proposal that sets :issue-certification? true -> HOLD, PERMANENT, never reaches request-approval -- this actor is never the legal-metrology/machinery-safety certification authority"
    (let [[db actor] (fresh)
...
(deftest invalid-calibration-accuracy-percent-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:calibration-accuracy-percent 999999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-calibration-accuracy-percent} (-> (store/ledger db) last :basis)))
    (is (not= 999999.0 (:calibration-accuracy-percent (store/batch db "batch-001"))) "fabricated calibration-accuracy-percent never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "asm-001" :severity :moderate
                                            :description "conveyor-drive pressure anomaly"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "asm-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
