(ns weighpkgmfg.sim
  ...
  calibration-line equipment directly (permanently blocked, no
  override), a proposal that tries to self-issue an OIML R76/NIST
  Handbook 44 legal-for-trade weighing-accuracy certification or a
  CE/ANSI-PMMI machinery-safety conformity mark (permanently blocked,
  no override), a double-schedule of the same maintenance window, a
  production-batch patch with a fabricated product type, a
  production-batch patch with an implausible calibration-accuracy
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on asm-001 (verified, registered, assembly line -- escalates, approve) ==")
    ...
    (println "== flag-safety-concern concern-1 on asm-001 (always escalates -- approve) ==")
    ...
    (println "== schedule-maintenance mnt-2 on bench-002 (UNVERIFIED/unregistered calibration-test bench -> HARD hold) ==")
    ...
    (println "== schedule-maintenance mnt-3 on asm-001 with :actuate-equipment? true -> HARD hold, PERMANENT, never reaches a human ==")
    ...
    (println "== log-production-batch batch-001 with an implausible calibration-accuracy reading -> HARD hold ==")
    ...
    (println "== log-production-batch batch-001 attempting to self-issue a legal-metrology/machinery-safety certification -> HARD hold, PERMANENT ==")
