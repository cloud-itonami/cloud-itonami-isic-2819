(ns weighpkgmfg.advisor
  "WeighPkgAdvisor -- the *contained intelligence node* for the
  general-purpose-machinery (ISIC 2819) plant-operations coordination
  actor, illustrated here with industrial weighing/packaging-machinery
  manufacturing (platform scales, checkweighers, batching scales,
  filling machines, wrapping machines, labeling machines).

  It normalizes production-batch patches (product-type/calibration-
  accuracy-percent/quantity/defect-rate), drafts an assembly/
  calibration-test-bench-equipment maintenance scheduling proposal
  against a piece of equipment, drafts a safety-concern flag, and
  drafts an outbound weighing/packaging-machinery shipment coordination
  ...
  assembly/calibration-line-equipment actuation, freight dispatch, or
  legal-metrology/machinery-safety certification -- see README `What
  this actor does NOT do`. Every output is censored downstream by
  `weighpkgmfg.governor` before anything touches the SSoT.
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `weighpkgmfg.governor`)
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; calibration-line-equipment-control
                                 ; effect
  ...
  `weighpkgmfg.governor` HARD-holds any request that doesn't, so a
...
(defn- log-production-batch
  ...
  calibration-accuracy-percent, quantity, defect-rate or verification
...
(defn- schedule-maintenance
  "Draft an assembly/calibration-test-bench-equipment maintenance-
  ...
  rationale, but `weighpkgmfg.governor` NEVER trusts this report -- it
...
(defn- flag-safety-concern
  ...
  `weighpkgmfg.phase`: no phase ever adds this op to a phase's `:auto`
  set; `weighpkgmfg.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound weighing/packaging-machinery shipment coordination
  ...
  `weighpkgmfg.governor` NEVER trusts it: it independently recomputes
...
(def ^:private system-prompt
  (str "あなたは計量・包装機器(はかり・自動チェッカー・バッチングスケール・"
       "充填機・包装機・ラベリングマシン等の一般産業用機械)製造工場プラント"
       "運用コーディネーターの助言者です。"
       ...
       "組立・校正試験等の設備の直接操作(actuate)を絶対に提案してはいけません"
       "(この actor は提案のみを行い、実行は一切行いません)。"
       "計量法定証明(OIML R76/NIST Handbook 44等)・機械安全適合性"
       "(CE/ANSI-PMMI B155.1等)を自己発行する提案をしてはいけません。"
       "出荷数量を偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `weighpkgmfg.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :weighpkgmfg-advisor-proposal
   :op         (:op request)
