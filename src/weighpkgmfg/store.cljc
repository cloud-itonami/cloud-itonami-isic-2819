(ns weighpkgmfg.store
  "SSoT for the general-purpose-machinery (ISIC 2819) plant-operations
  coordination actor, illustrated with industrial weighing/packaging-
  ...
  `fluidpowermfg.store`, `cloud-itonami-isic-2814`'s own
  `beargearmfg.store`), this build ships a single `MemStore` backend
  ...
  is self-contained (no external general-purpose-machinery-
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-type/calibration-accuracy-percent/
    ...
    - `equipment`         -- an assembly-line/calibration-test-bench
    ...
    - `maintenance`       -- a scheduled assembly/calibration-test-
                             bench-equipment maintenance-window DRAFT
                             against a piece of equipment
                             (`weighpkgmfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound weighing/packaging-
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (weighpkgmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (weighpkgmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :platform-scale
                :model "PS-A500" :calibration-accuracy-percent 0.3
                :quantity-units 4000.0 :defect-rate-percent 0.6
                :verified? true :registered? true
                :shipped-units 900.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-type :checkweigher
                :model "CW-T12" :calibration-accuracy-percent 0.15
                :quantity-units 1200.0 :defect-rate-percent 0.4
                :verified? true :registered? true
                :shipped-units 1150.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-type :filling-machine
                :model "FM-R80" :calibration-accuracy-percent 1.2
                :quantity-units 800.0 :defect-rate-percent 1.1
                :verified? false :registered? false
                :shipped-units 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"asm-001" {:id "asm-001" :kind :assembly-line
              :verified? true :registered? true
              :last-maintenance-date "2026-05-01"}
   "bench-002" {:id "bench-002" :kind :calibration-test-bench
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `weighpkgmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `weighpkgmfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  assembly-line unit (schedulable for maintenance), one
  UNVERIFIED/unregistered calibration-test-bench unit (blocks any
