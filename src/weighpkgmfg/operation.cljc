(ns weighpkgmfg.operation
  "WeighPkgOperationActor -- one plant-operations coordination request
  = one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (WeighPkgAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the General-Purpose
...
(defn build
  "Compiles a WeighPkgOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `weighpkgmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; WeighPkgAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; General-Purpose Machinery Plant Operations Governor -- independent censor.
      (g/add-node :govern
