(ns weighpkgmfg.governor
  "General-Purpose Machinery Plant Operations Governor -- the
  independent compliance layer that earns the WeighPkgAdvisor the
  ...
  draft-schedule) assembly/calibration-line equipment, whether a
  proposal secretly tries to self-issue an OIML R76/NIST Handbook 44
  legal-for-trade weighing-accuracy certification or a CE/ANSI-PMMI
  machinery-safety conformity mark (an authority this actor never
  holds), whether a shipment proposal's own claimed quantity would
  blow through the batch's own logged production quantity, or when an
  act stops being a coordination proposal and becomes direct assembly/
  calibration-line-equipment control, so this MUST be a separate
  system able to *reject* a proposal and fall back to HOLD.

  `:itonami.blueprint/governor` is
  `:general-purpose-machinery-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       actuate` or `:calibration-bench/
                                       run`) is the 'direct assembly/
                                       calibration-line-equipment
    ...
    5. Certification-authority
       blocked                     -- ANY proposal (any op) whose own
                                       ...
                                       attempting to self-issue an OIML
                                       R76/NIST Handbook 44 legal-for-
                                       trade weighing-accuracy
                                       certification or a CE/ANSI-PMMI
                                       machinery-safety conformity mark
                                       -- an authority exclusively
                                       reserved to the accredited
                                       legal-metrology/machinery-safety
   ...
   13. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [weighpkgmfg.registry :as registry]
...
(def allowed-proposal-effects
  ...
  calibration-line-equipment-control effect."
...
(defn- equipment-control-blocked-violations
  ...
  Anything else (direct assembly/calibration-line-equipment control, a
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :equipment-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") は組立・校正試験設備の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- equipment-actuate-blocked-violations
  ...
  to directly actuate assembly/calibration-line equipment -- this
  ...
  (when (and (= op :schedule-maintenance)
    ...
    [{:rule :equipment-actuate-blocked
      :detail "組立・校正試験設備の直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn- certification-authority-blocked-violations
  ...
  to self-issue an OIML R76/NIST Handbook 44 legal-for-trade weighing-
  accuracy certification or a CE/ANSI-PMMI machinery-safety conformity
  mark -- an authority exclusively reserved to the accredited legal-
  metrology/machinery-safety certification body, never this actor. No
  ...
  (let [payload (or (:value proposal) (:patch proposal))]
    (when (true? (:issue-certification? payload))
      [{:rule :certification-authority-blocked
        :detail "計量法定証明(OIML R76/NIST Handbook 44等)・機械安全適合性(CE/ANSI-PMMI B155.1等)の自己発行提案は恒久的に禁止 -- 認証機関の専権事項"}])))
...
(defn- invalid-calibration-accuracy-percent-violations
  "For `:log-production-batch`, if the patch declares a
  `:calibration-accuracy-percent` that is not a physically plausible
  calibration-test reading, reject rather than let fabricated/sensor-
  error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [pct (:calibration-accuracy-percent (:value proposal))]
      (when (and (some? pct) (not (registry/calibration-accuracy-percent-valid? pct)))
        [{:rule :invalid-calibration-accuracy-percent
          :detail (str pct "% は物理的に妥当な校正精度値の範囲外")}]))))
...
(defn check
  "Censors a WeighPkgAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (invalid-calibration-accuracy-percent-violations request proposal)
